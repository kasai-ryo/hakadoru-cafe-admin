-- ハカドルカフェ テストデータ (認証なし開発用)
-- 作成日: 2025-11-07
-- バージョン: 1.0
--
-- このスクリプトは開発・テスト用です
-- Supabase Authの外部キー制約を一時的に無効化してテストデータを投入します

-- ==================================================
-- 注意事項
-- ==================================================
-- 1. このスクリプトを実行する前に、DDL (ddl.sql) を実行してテーブルを作成してください
-- 2. このスクリプトは開発環境でのみ使用してください
-- 3. 本番環境では必ずSupabase Authでユーザーを作成してから使用してください

-- ==================================================
-- 外部キー制約を一時的に無効化
-- ==================================================
ALTER TABLE accounts DROP CONSTRAINT IF EXISTS accounts_id_fkey;
ALTER TABLE ssoaccounts DROP CONSTRAINT IF EXISTS ssoaccounts_account_id_fkey;
ALTER TABLE accounts_plans DROP CONSTRAINT IF EXISTS accounts_plans_account_id_fkey;
ALTER TABLE reports DROP CONSTRAINT IF EXISTS reports_account_id_fkey;
ALTER TABLE favorites DROP CONSTRAINT IF EXISTS favorites_account_id_fkey;
ALTER TABLE cafe_requests DROP CONSTRAINT IF EXISTS cafe_requests_account_id_fkey;
ALTER TABLE cafe_edit_requests DROP CONSTRAINT IF EXISTS cafe_edit_requests_account_id_fkey;
ALTER TABLE notifications DROP CONSTRAINT IF EXISTS notifications_account_id_fkey;
...
--
-- 注意: このスクリプトは開発環境専用です
-- 本番環境では必ずSupabase Authを使用してください
