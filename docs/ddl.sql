...
CREATE TABLE ssoaccounts (
    ...
    provider VARCHAR(50) NOT NULL CHECK (
        provider IN ('google', 'twitter', 'line')
    ),
    created_at TIMESTAMP NOT NULL DEFAULT NOW()
);
...
CREATE TABLE accounts_plans (
    ...
    plan_status VARCHAR(20) NOT NULL DEFAULT 'active' CHECK (
        plan_status IN (
            'active',
            'trialing',
            'canceled',
...
CREATE TABLE reports (
    ...
    spaciousness INT CHECK (
        spaciousness >= 1
        AND spaciousness <= 5
    ),
    power_availability INT CHECK (
        power_availability >= 1
        AND power_availability <= 5
    ),
...
CREATE POLICY accounts_select_policy ON accounts FOR
SELECT USING (auth.uid () = id);

CREATE POLICY accounts_update_policy ON accounts FOR
UPDATE USING (auth.uid () = id);

-- ssoaccounts: SELECT (自分の情報のみ)
CREATE POLICY ssoaccounts_select_policy ON ssoaccounts FOR
SELECT USING (auth.uid () = account_id);

-- accounts_plans: SELECT/UPDATE (自分の情報のみ)
CREATE POLICY accounts_plans_select_policy ON accounts_plans FOR
SELECT USING (auth.uid () = account_id);

CREATE POLICY accounts_plans_update_policy ON accounts_plans FOR
UPDATE USING (auth.uid () = account_id);

-- cafes: SELECT (全ユーザー閲覧可能)
CREATE POLICY cafes_select_policy ON cafes FOR SELECT USING (true);

-- cafe_images: SELECT (全ユーザー閲覧可能)
CREATE POLICY cafe_images_select_policy ON cafe_images FOR
SELECT USING (true);

-- reports: SELECT (全ユーザー閲覧可能)
CREATE POLICY reports_select_policy ON reports FOR
SELECT USING (true);

-- reports: INSERT (ログインユーザのみ)
CREATE POLICY reports_insert_policy ON reports FOR
INSERT
WITH
    CHECK (auth.uid () = account_id);

-- reports: UPDATE/DELETE (自分の投稿のみ)
CREATE POLICY reports_update_policy ON reports FOR
UPDATE USING (auth.uid () = account_id);

CREATE POLICY reports_delete_policy ON reports FOR DELETE USING (auth.uid () = account_id);

-- favorites: SELECT (自分のお気に入りのみ)
CREATE POLICY favorites_select_policy ON favorites FOR
SELECT USING (auth.uid () = account_id);

-- favorites: INSERT/DELETE (自分のお気に入りのみ)
CREATE POLICY favorites_insert_policy ON favorites FOR
INSERT
WITH
    CHECK (auth.uid () = account_id);

CREATE POLICY favorites_delete_policy ON favorites FOR DELETE USING (auth.uid () = account_id);

-- cafe_requests: SELECT (自分のリクエストのみ)
CREATE POLICY cafe_requests_select_policy ON cafe_requests FOR
SELECT USING (auth.uid () = account_id);

-- cafe_requests: INSERT (ログインユーザのみ)
CREATE POLICY cafe_requests_insert_policy ON cafe_requests FOR
INSERT
WITH
    CHECK (auth.uid () = account_id);

-- cafe_edit_requests: SELECT (自分のリクエストのみ)
CREATE POLICY cafe_edit_requests_select_policy ON cafe_edit_requests FOR
SELECT USING (auth.uid () = account_id);

-- cafe_edit_requests: INSERT (ログインユーザのみ)
CREATE POLICY cafe_edit_requests_insert_policy ON cafe_edit_requests FOR
INSERT
WITH
    CHECK (auth.uid () = account_id);

-- notifications: SELECT (自分の通知のみ)
CREATE POLICY notifications_select_policy ON notifications FOR
SELECT USING (auth.uid () = account_id);

-- notifications: UPDATE (自分の通知のみ)
CREATE POLICY notifications_update_policy ON notifications FOR
UPDATE USING (auth.uid () = account_id);
